...
class TTTState:
	def __init__(self, board, ptm):
		"""
		board is represented as a 2D List of character strings.
        Each character in the board is X, O, or SPACE (see above
        for global definition), where SPACE indicates
        that the corresponding cell of the tic-tac-toe board is empty.

        ptm is the index of the player to move, which will be 0 or 1,
...
class TTTProblem(AdversarialSearchProblem):
	...
	def __init__(self, dim=3, board=None, player_to_move=0):
		"""
		- dim is the number of cells in one row or column.
		- board and player_to_move are for the start state
		"""
		self._dim = dim
		if board == None:
